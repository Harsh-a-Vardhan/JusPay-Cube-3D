...
cubes =
    ...
    where
        ...
        handleAction query = case query of
            ...
            IncVelocity number -> runFunction (\c -> map (incVelocity number) c)
            DecVelocity number -> runFunction (\c -> map (decVelocity number) c)
...
incVelocity :: Number -> Cube -> Cube
incVelocity id c = if id == c.id then c{speed = 2.0*c.speed} else c

decVelocity :: Number -> Cube -> Cube
decVelocity id c = if id == c.id then c{speed = 0.5*c.speed} else c
...
renderView state = let
    ...
    in
        ...
        , renderButton "Vel++"(IncVelocity state.id)
        , renderButton "Vel--"(DecVelocity state.id)
        , renderButton "AddCube"(AddCube)
        , renderButton "RemoveCube"(RemoveCube)
        , HH.text ("Direction"<> show state.forward)
        , HH.text ("Speed"<> show state.speed)
        ]
        <>
        [ SE.svg
