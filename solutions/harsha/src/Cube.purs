...
cubes =
    ...
    where
        ...
        handleAction query = case query of
            ...
            DeleteCube -> runFunction (\c -> drop 1 c)
...
reverse id cube = 
    if (id == cube.id) then cube { forward = not cube.forward } else cube

incAngVelocity :: Axis -> Cube -> Cube
incAngVelocity axis c = do 
  let {xa, ya, za} = c.angVel
  case axis of
    X -> c { angVel { xa = if c.forward then xa + accelerateBy + c.speed else xa - accelerateBy - c.speed} }
    Y -> c { angVel { ya = if c.forward then ya + accelerateBy + c.speed else ya - accelerateBy - c.speed} }
    Z -> c { angVel { za = if c.forward then za + accelerateBy + c.speed else za - accelerateBy - c.speed} }

increaseSpeed ::  Int -> Cube -> Cube
increaseSpeed id cube = if id==cube.id then cube{speed = cube.speed + 1.0 } else cube

decreaseSpeed :: Int -> Cube -> Cube
decreaseSpeed id cube = if id==cube.id then cube{speed= cube.speed - 1.0 } else cube
...
renderView state = let
    ...
    in
        ...
        , renderButton "Speed+" (IncreaseSpeed state.id)
        , renderButton "Speed-" (DecreaseSpeed state.id)
        ]
        <>
        [ SE.svg
