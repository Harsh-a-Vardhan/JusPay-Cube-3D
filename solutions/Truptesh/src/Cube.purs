...
import Prelude
import Data.Tuple
import Data.Array (mapWithIndex,take,snoc,(!!), length)
...
cubes =
    ...
    where
        ...
        handleAction query = case query of
            ...
            ReverseDirection id -> runFunction  (\c -> map (reverseDirection id) c)
            IncVelocity  id -> runFunction  (\c -> map (incVelocity id) c)
            DecVelocity  id -> runFunction  (\c -> map (decVelocity id) c)
...
incAngVelocity axis c = do
  ...
  case axis of
    ...
    X -> c { angVel { xa = if c.forward == true then xa + accelerateBy * c.speed else xa - accelerateBy * c.speed } }
    Y -> c { angVel { ya = if c.forward == true then ya + accelerateBy * c.speed else ya - accelerateBy * c.speed } }
    Z -> c { angVel { za = if c.forward == true then za + accelerateBy * c.speed else za - accelerateBy * c.speed } }


reverseDirection :: Int -> Cube -> Cube
reverseDirection id c= do
  if id == c.id then c {forward = not c.forward} else c
incVelocity :: Int -> Cube -> Cube
incVelocity id c = do
  if id == c.id then c {speed = 2.0 * (c.speed)} else c
decVelocity :: Int -> Cube -> Cube
decVelocity id c= do
  if id == c.id then c {speed = 0.5 * (c.speed)} else c
tick :: Cube -> Cube
tick c =  do
  let angVel = c.angVel
...
renderView state = let
    ...
    in
        ...
        , renderButton "Reverse" (ReverseDirection state.id)
        , renderButton "Speed Increase" (IncVelocity state.id)
        , renderButton "Speed Decrease" (DecVelocity state.id)
        , renderButton "Add" ( AddCube )
        , renderButton "Remove" (RemoveCube)
        ]
        <>
        [ SE.svg
