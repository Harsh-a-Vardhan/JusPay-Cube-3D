...
import Data.Tuple
import Data.Array (mapWithIndex, (!!),snoc,drop,length)
        ...
        handleAction query = case query of
            ...
            Addition -> do
                  _ <- H.modify (\c ->
                         ...
                         snoc c (initCube { num = (length c) + 1 } )
                  ) 
                  pure unit  
...
incAngVelocity axis cube = do
                    let {xa, ya, za} = cube.angVel
                    let increment = cube.increment
                    case axis of 
                            X -> cube { angVel { xa = if cube.forward then xa + accelerateBy + increment  else xa - accelerateBy - increment}}
                            Y -> cube { angVel { ya = if cube.forward then ya + accelerateBy + increment else ya - accelerateBy - increment}}
                            Z -> cube { angVel { za = if cube.forward then za + accelerateBy + increment else za - accelerateBy - increment}}
                        
reversefn :: Int -> RotatingShape -> RotatingShape
reversefn num cube = do
                    if cube.num == num then cube {forward = not cube.forward }
                    else cube

speedi :: Int -> RotatingShape -> RotatingShape
speedi num cube = do
                    let si = cube.increment
                    if cube.num == num then cube { increment =  si + 1.0 }
                    else cube

speedd :: Int -> RotatingShape -> RotatingShape
speedd num cube = do
                    let si = cube.increment
                    if cube.num == num then cube { increment =  si - 1.0 }
                    else cube
...
renderView state = let
    ...
    in
        ...
        , renderButton "reverse" (Reversefn state.num)
        , renderButton "speed++" (SpeedI state.num)
        , renderButton "speed--" (SpeedD state.num)
        , renderButton "Add" (Addition)
        , renderButton "Remove" (Removal)
        ]
        <>
        [ SE.svg
