...
cubes =
   ...
   where
       ...
       handleAction query = case query of
           ...
           Reverse -> runFunction  (\c -> map(reverse) c)
           SpeedAngVelocity -> runFunction (\c -> map(speedAngVelocity) c)
           DampenVelocity -> runFunction (\c -> map(dampenVelocity) c)
...
speedAngVelocity :: Cube -> Cube
speedAngVelocity c = c {speedBy = c.speedBy + 20.0}
 
dampenVelocity :: Cube -> Cube
dampenVelocity c = c {speedBy = c.speedBy - 20.0}
...
renderView state = let
   ...
   in
       ...
       , renderButton "speed++" (SpeedAngVelocity)
       , renderButton "speed--" (DampenVelocity)
