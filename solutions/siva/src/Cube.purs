...
import Data.Array (drop, length, mapWithIndex, snoc, take, (!!))
...
import Halogen.Svg.Attributes (c)
...
cubes =
  ...
  where
      ...
      handleAction query = case query of
          ...
          SpeedAngVelocity id -> runFunction (\c -> map(speedAngVelocity id) c)
          DampenVelocity id -> runFunction (\c -> map(dampenVelocity id) c)
          AddCube -> runFunction  (\c -> snoc c initCube{identifier = length c + 1})
...
incAngVelocity axis c = do
    ...
    case axis of
        X -> c { angVel { xa = if c.forward then xa + accelerateBy + c.speedBy else xa - accelerateBy - c.speedBy } }
        Y -> c { angVel { ya = if c.forward then ya + accelerateBy + c.speedBy else ya - accelerateBy - c.speedBy } }
        Z -> c { angVel { za = if c.forward then za + accelerateBy + c.speedBy else za - accelerateBy - c.speedBy } }
    
reverse :: Int -> Cube -> Cube
reverse id c = if (id == c.identifier) then c {forward = not c.forward} else c
 
speedAngVelocity :: Int -> Cube -> Cube
speedAngVelocity id c = if (id == c.identifier) then c {speedBy = c.speedBy + 20.0} else c
dampenVelocity :: Int -> Cube -> Cube
dampenVelocity id c = if ( id == c.identifier) then c {speedBy = c.speedBy - 20.0} else c
tick :: Cube -> Cube
tick c =  do
    let angVel = c.angVel
...
renderView state = let
  ...
  in
      ...
      , renderButton "reverse" (Reverse state.identifier)
      , renderButton "speed++" (SpeedAngVelocity state.identifier)
      , renderButton "speed--" (DampenVelocity state.identifier)
